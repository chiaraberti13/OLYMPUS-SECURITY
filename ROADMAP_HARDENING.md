...
| **Hermes** (secret scan) | ~~Baseline~~, ~~allowlist~~, ~~entropy detection~~, ~~output SARIF~~ (tutti fatti); ancora aperto: hook pre-commit/CI | `P1` |
...
| **Metis** (CTI) | ~~**STIX 2.1** export/import~~, ~~**MISP** export/import~~ (fatti: `olympus.metis.stix`/`.misp`, round-trip deterministico, sottoinsieme fedele con skip espliciti); ancora aperti: TAXII (feed remoto), backup/restore, cifratura campi sensibili | `P1` |
