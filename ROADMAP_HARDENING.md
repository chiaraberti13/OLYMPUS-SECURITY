...
|---|---|---|
| **Argus** (OSINT/recon) | ~~correlazione entità + analisi grafo~~ (fatto: `olympus.argus.correlation` + `argus correlate` — componenti connesse, hub di pivot per grado, vicinato n-hop, correlazione per valore condiviso); ancora aperto: arricchimento IOC da feed esterni (rete) | `P1` |
...
| **Minerva** (IR/chain-of-custody) | ~~Ledger firmato **HMAC**~~ (HMAC-SHA256, schema 2.1.0, rileva truncation/rewrite), ~~firma **Ed25519**~~ (fatto: `core.signing` + `core sign`/`verify`, verifica di terze parti con chiave pubblica pinnata — firma detached su ledger/evidenze/report/SBOM); ancora aperti: **trusted timestamp** (RFC 3161) e Ed25519 inline nel percorso di append del ledger | `P1` |
...
- [~] `P2` Firma delle immagini e provenance, oltre a SBOM e vulnerability scan (§1.3). **Provenance firmabile fatta:** `olympus core keygen`/`sign`/`verify` (Ed25519 detached, verifica di terze parti con chiave pinnata) firma qualsiasi artefatto — SBOM, lockfile, report, ledger, evidenze. **Aperto:** firma delle *immagini* container (cosign/notation) — richiede build/registry (Docker, bloccato qui).
...
| 2026-09-11 | **§2/Metis — backup/restore cifrati** | test locali verdi, CI da confermare | `metis case backup --encrypt` cifra lo snapshot consistente dell'intero store (via file temporaneo privato → `core.crypto.encrypt_bytes`) sotto `OLYMPUS_METIS_KEY`, owner-only; `metis case restore` **autodetecta** un envelope cifrato (magic SQLite vs JSON) e lo decifra prima di verificare/ripristinare. Chiave errata/tamper rifiutati; il backup in chiaro continua a funzionare senza chiave. Completa la coppia backup/restore + cifratura per backup offsite sicuri. 3 nuovi test. Ruff pulito, mypy pulito sui moduli toccati, 1399 test |
| 2026-09-11 | **§2/Minerva + §5.3 — firma Ed25519 (provenance)** | test locali verdi, CI da confermare | `olympus.core.signing` + `olympus core keygen`/`sign`/`verify`: firma **Ed25519 detached** su qualsiasi artefatto (ledger, evidenze, SBOM, report) con **verifica di terze parti** — chi verifica usa solo la chiave pubblica, e la verifica è **pinnata** alla chiave fidata (un attaccante che ri-firma con la propria chiave è rifiutato). Envelope JSON autodescrittivo, chiave privata owner-only. Chiude la metà «Ed25519/verifica di terze parti» del gap «Signed evidence ledger» del threat model (aggiornato: nuova riga di controllo + gap ristretto al solo trusted timestamp RFC 3161). Sbloccato dalla dipendenza `cryptography`. 8 test. Ruff pulito, 1407 test |
| 2026-09-11 | **§2/Argus — correlazione entità + analisi grafo** | test locali verdi, CI da confermare | `olympus.argus.correlation` + `argus correlate <investigation.json>`: analisi deterministica e dependency-free del grafo investigativo — **componenti connesse** (cluster del caso), **pivot** per grado (hub da cui pivotare), **vicinato n-hop** da un seed, e **correlazione per valore condiviso** (link non-ovvi: stesso valore come `value` di un'entità e attributo di un'altra, es. account↔email). Aggiunto `investigation_from_dict` per il round-trip del grafo persistito. Manifest di parità CLI e conteggio comandi TUI aggiornati (drift-guard). 7 test. Ruff pulito, mypy pulito sui moduli toccati, 1414 test |
| 2026-09-11 | **Security self-review — DoS KDF in `core.crypto`** | test locali verdi, CI da confermare | Auto-review del codice security-critical aggiunto: trovato e corretto un **memory-bomb**. `decrypt_to_bytes` leggeva i parametri scrypt (`n`/`r`/`p`) da un envelope **non fidato** e li passava a `Scrypt` senza limiti (n grande → allocazione illimitata). Aggiunto `_validate_kdf_params` (n potenza-di-due entro `2**21`, tetto memoria `128·n·r ≤ 1 GiB`, r/p/salt limitati) applicato **prima** di derivare la chiave. 5 test di regressione parametrici. Ruff/mypy puliti, 1419 test |
| 2026-09-11 | **Security self-review — mistranslation in import Sigma** | test locali verdi, CI da confermare | Secondo finding dell'auto-review: `apollo.sigma._conditions_from_detection` **stringificava** un valore di campo che era una mappa annidata (`Image:\n  sub: nested`) in un match esatto spazzatura (`"{'sub': 'nested'}"`) invece di rifiutarlo — violava il contratto «importa il sottoinsieme fedele, rifiuta il resto». Aggiunto il rifiuto esplicito delle mappe annidate come valore. 1 test di regressione parametrico. Ruff/mypy puliti, 1420 test |
| 2026-09-11 | **Security self-review — stessa classe di bug in import MISP** | test locali verdi, CI da confermare | Terzo finding: `metis.misp.event_to_indicators` faceva `str(value)` su un valore di attributo non-stringa (evento MISP malformato/ostile con `value` mappa/lista) creando un IOC spazzatura (`domain: "{'nested': 1}"`). Aggiunto lo **skip con motivo** dei valori non-stringa (STIX era già sicuro: valori da capture regex). 1 test di regressione. Ruff/mypy puliti, 1421 test |
| 2026-09-11 | **Security review moduli preesistenti — gap proxy nel pinning** | test locali verdi, CI da confermare | Estesa la review ai controlli preesistenti. **SSRF guard `core.addresses` verificata solida** (22 casi avversariali: metadata cloud, wrapper IPv4-in-IPv6 mapped/compatible/NAT64/6to4, CGNAT, ULA, link-local, multicast — nessun bypass; nessun finding). **Finding nel pinning:** l'opener di `core.http` per le richieste pinnate ereditava il `ProxyHandler` di default che legge i proxy d'ambiente; il tunnel HTTPS CONNECT era già rifiutato, ma un `HTTP_PROXY` semplice avrebbe **silenziosamente aggirato il pin** (il proxy ri-risolve il target). Fix: `ProxyHandler({})` esplicito sull'opener pinnato → nessun proxy per alcuno schema, garanzia indipendente dall'ambiente; gli opener non-pinnati restano invariati. 2 test di regressione. 1423 test |
