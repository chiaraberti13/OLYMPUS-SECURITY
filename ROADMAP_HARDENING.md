...
- [~] `P2` Generare SBOM e scansione vulnerabilità in CI (vedi §3.5, Syft/Grype/Trivy). **SBOM fatto**: `olympus core sbom` genera CycloneDX 1.5 nativo (da `importlib.metadata`, senza tool esterni) e la CI lo produce dal wheel e lo pubblica come artefatto. Scansione vulnerabilità (Grype/Trivy) ancora da aggiungere.
